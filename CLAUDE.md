...
| EMAIL.FIX4 | HAUTE | `IntegrationsTab.tsx` + `microsoft-connect` | Après connexion Microsoft OAuth réussie (code échangé, tokens stockés), la page Settings affiche toujours "non connecté". **3 causes** : (1) Race condition — le fetch `/api/emails/get-connection` se fait avant que les données async (profile, connection) ne soient disponibles ; un seul retry à 1.5s pas suffisant. (2) Le INSERT `email_connections` pouvait échouer silencieusement (contrainte unique) sans empêcher le redirect `?connected=email`. (3) Pas d'état loading visible pendant la vérification, l'utilisateur voit les boutons "non connecté" | **Fix** : (1) IntegrationsTab : état `justConnected` qui affiche un loader au lieu des boutons provider, retry agressif (5 tentatives × 1.5s) via `setInterval` + détection via `isLegacyOutlook` quand le profil charge. (2) Microsoft-connect : DELETE anciennes connections avant INSERT (évite contrainte unique). (3) URL cleanup seulement après confirmation de connexion ou timeout |
