...
| `/api/visits/generate-report` | POST | Générer rapport (inclut notes manuscrites transcrites) |
| `/api/visits/export-report` | POST | Exporter rapport DOCX (inclut photos + transcriptions) |
| `/api/visits/photos/upload` | POST | Upload photo (FormData), stocke dans bucket `audio` path `photos/{orgId}/{visitId}/` |
| `/api/visits/photos` | GET | Liste photos d'une visite (`?visit_id=`) |
| `/api/visits/photos/[id]` | PATCH | Mettre à jour caption/location_description |
| `/api/visits/photos/[id]` | DELETE | Supprimer photo (Storage + DB, trigger met à jour photos_count) |
| `/api/visits/analyze-notes` | POST | Analyse Claude Vision d'une photo de notes manuscrites (maxDuration=120) |
...
- `visits/PhotoCapture.tsx` — Capture photo mobile-first (camera + upload, compression Canvas API max 2048px)
- `visits/PhotoGallery.tsx` — Galerie photos avec lightbox, caption éditable, badges type
- `visits/HandwrittenNotesResult.tsx` — Affichage résultat analyse notes manuscrites (transcription, croquis, mesures)
...
### Visits (`visits/`, 4 fichiers)
- `transcription-service.ts` — `transcribeVisitAudio()` (Whisper)
- `visit-report-generator.ts` — `buildVisitReportPrompt()`, `getMockVisitReport()` (accepte `handwritten_notes` + `sketch_descriptions`)
- `handwritten-notes-analyzer.ts` — `analyzeHandwrittenNotes()` (Claude Vision, transcription écriture manuscrite FR/DE/EN, croquis, mesures)
...
### Corrigés — Settings & Super-Admin (2026-03-15)
...
| PROFILE.FIX3 | HAUTE | `auth/callback` Check 2 | Microsoft OAuth ne fournit que `full_name` dans `user_metadata`, pas `first_name`/`last_name` séparés. Lors de la re-connexion (Check 2), le callback ne synchronisait pas les métadonnées Auth avec les données DB | Ajouté sync `user_metadata` depuis la table `users` via `admin.auth.admin.updateUserById()` quand `first_name`/`last_name` manquent dans les métadonnées |
...
| EMAIL.FIX4 | HAUTE | `IntegrationsTab.tsx` + `microsoft-connect` | Après connexion Microsoft OAuth réussie (code échangé, tokens stockés), la page Settings affiche toujours "non connecté". **4 causes** : (1) Race condition — `useUserProfile(undefined)` set `loaded: true` immédiatement quand userId est undefined, mais le `useEffect` de fetch connection renvoie early sans `user?.id`, donc `checked` reste false. (2) `loading = !checked && !hasConnection` restait true indéfiniment (spinner infini). (3) Le INSERT `email_connections` pouvait échouer silencieusement (contrainte unique). (4) Le redirect_uri devait matcher exactement le domaine Azure AD enregistré | **Fix** : (1) `loading = !checked && !hasConnection && !profile?.loaded` — quand le profile hook a terminé (même sans données), on arrête le spinner et on affiche les boutons provider. Quand auth résout et `user?.id` arrive, le `useEffect` fetch la connexion et met à jour l'UI. (2) Microsoft-connect : DELETE avant INSERT, vérification post-UPDATE, redirect_uri depuis headers request. (3) get-connection : fallback synthétique depuis `users.microsoft_access_token` |
| EMAIL.FIX2 | HAUTE | `auth/callback` redirect | Après connexion email OAuth (flow `linkOrgId`), le callback redirige vers `/settings` sans paramètre `connected=email` et sans sélectionner l'onglet Connexion Email — l'utilisateur atterrit sur l'onglet Profil sans rafraîchissement | Ajouté `?tab=outlook&connected=email` au redirect quand `linkOrgId` est présent |
| EMAIL.FIX3 | CRITIQUE | Split identity | Quand `linkIdentity` échoue (identité Azure déjà liée à un autre auth user), le fallback `signInWithOAuth` change la session browser vers l'autre auth user. Le callback redirigait vers `/login` et la connexion email n'apparaissait pas | **3 corrections** : (1) `get-connection` cherche par `organization_id` en fallback quand aucune connexion trouvée par `user_id` (gère le split identity). (2) Auth callback split identity : crée la connexion sous BOTH users, copie le profil original vers l'OAuth user, sync `user_metadata`, `onboarding_completed: true`, redirige vers settings (pas login), handler entier dans try/catch non-fatal. (3) `signInWithMicrosoftAction` : cleanup complet de l'orphelin Azure (migration FK sur 13 tables + suppression logs sur 3 tables) avant `deleteUser` et retry `linkIdentity` |
...
## 21. Photos & Notes Manuscrites dans Visites Client (2026-03-15)

### Fonctionnalité
Les chefs de projet peuvent maintenant capturer des photos lors des visites client :
- **Photos de chantier** (`site`) : documentation visuelle du chantier
- **Notes manuscrites** (`handwritten_notes`) : photos de notes papier analysées par IA (Claude Vision)

### Architecture

#### Table `visit_photos` (migration 053)
- `photo_type`: `site` | `handwritten_notes`
- `ai_analysis_status`: `pending` | `processing` | `completed` | `failed`
- `ai_transcription`, `ai_sketch_description`, `ai_analysis_result` (JSONB)
- Trigger `trg_visit_photos_count` auto-met à jour `client_visits.photos_count`
- RLS standard par `organization_id`

#### Analyse IA notes manuscrites
- `packages/core/src/visits/handwritten-notes-analyzer.ts`
- Claude Vision (Sonnet 4.5) avec prompt spécialisé construction FR/DE/EN
- Retourne : `transcribed_text`, `sketches[]`, `measurements_found[]`, `language_detected`, `confidence`
- Résultats agrégés dans `client_visits.handwritten_notes_transcription`
- Notes intégrées dans la génération de rapport (`generate-report`) et l'export DOCX (`export-report`)

#### Composants UI
- `PhotoCapture.tsx` — Mobile-first (`capture="environment"`), compression Canvas API (max 2048px, JPEG 85%), batch upload avec progress
- `PhotoGallery.tsx` — Grille 3/2 colonnes, lightbox plein écran, caption éditable, download/delete
- `HandwrittenNotesResult.tsx` — Photo + transcription côte à côte, croquis identifiés, mesures extraites (chips)

#### Intégration pages
- `/visits/new` step 2 : deux accordéons "Notes manuscrites" + "Photos du chantier" sous l'AudioRecorder
- `/visits/[id]` : nouvel onglet "Photos" avec badge count, sections notes + site photos
- `/visits` liste : icône caméra + badge count sur les visites avec photos

#### Stockage
- Bucket Supabase `audio`, path `photos/{orgId}/{visitId}/{uniqueName}`
- Formats acceptés : JPEG, PNG, WebP (max 10 MB)
- Filename sanitisé : `.replace(/[^a-zA-Z0-9._-]/g, "_")`

#### Sécurité
- Toutes les routes : auth + vérification `organization_id` (anti-IDOR)
- Pas de SVG (XSS prevention)
- Cast pattern : `((admin as any).from("visit_photos"))` (table pas dans le type Database)
...
5. Appliquer migrations 049-053 (submissions enhanced + budget + visit photos)
