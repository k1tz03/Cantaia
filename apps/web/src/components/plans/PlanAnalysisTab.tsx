    ...
    <div className="space-y-4">
      {analyzing && (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-[#27272A] bg-[#0F0F11] py-16">
          ...
          <p className="text-sm font-medium text-[#A1A1AA]">{t("analyzing")}</p>
          <p className="mt-1 text-xs text-[#A1A1AA]">L&apos;IA analyse le plan comme un metreur professionnel...</p>
        </div>
      )}
      ...
      {!analysis && !analyzing && !analysisError && (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-[#27272A] bg-[#0F0F11] py-16">
          <Sparkles className="h-12 w-12 text-[#A1A1AA] mb-3" />
          <p className="text-sm font-medium text-[#A1A1AA]">{t("noAnalysisYet")}</p>
          <p className="mt-1 text-xs text-[#A1A1AA] max-w-sm text-center">{t("noAnalysisDesc")}</p>
          <button
        ...
        <>
          ...
          {result.title_block && (
            <div className="rounded-lg border border-[#27272A] bg-[#0F0F11] p-4">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-[#A1A1AA] mb-3">{t("titleBlock")}</h3>
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-4">
                {result.title_block.plan_number && (
                  <div><p className="text-[10px] text-[#A1A1AA]">N&deg;</p><p className="font-mono font-medium text-[#FAFAFA]">{result.title_block.plan_number}</p></div>
                )}
                {result.title_block.plan_title && (
                  <div className="col-span-2 sm:col-span-3"><p className="text-[10px] text-[#A1A1AA]">Titre</p><p className="text-[#FAFAFA]">{result.title_block.plan_title}</p></div>
                )}
                {result.title_block.scale && (
                  <div><p className="text-[10px] text-[#A1A1AA]">Echelle</p><p className="text-[#FAFAFA]">{result.title_block.scale}</p></div>
                )}
                {result.title_block.date && (
                  <div><p className="text-[10px] text-[#A1A1AA]">Date</p><p className="text-[#FAFAFA]">{result.title_block.date}</p></div>
                )}
                {result.title_block.company && (
                  <div><p className="text-[10px] text-[#A1A1AA]">Bureau</p><p className="text-[#FAFAFA]">{result.title_block.company}</p></div>
                )}
                {result.title_block.revision && (
                  <div><p className="text-[10px] text-[#A1A1AA]">Indice</p><p className="font-mono font-bold text-[#FAFAFA]">{result.title_block.revision}</p></div>
                )}
              </div>
            </div>
          )}
          ...
          {result.legend_items.length > 0 && (
            <div className="rounded-lg border border-[#27272A] bg-[#0F0F11] p-4">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-[#A1A1AA] mb-3">{t("legendLabel")}</h3>
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
                {result.legend_items.map((item, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-xs text-[#A1A1AA]">&mdash; {item.description}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {result.quantities.length > 0 && (
            <div className="rounded-lg border border-[#27272A] bg-[#0F0F11] overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#27272A]">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-[#A1A1AA]">{t("quantitiesLabel")}</h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#27272A] bg-[#27272A]/50">
                      <th className="px-4 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-[#A1A1AA]">{t("colItem")}</th>
                      <th className="px-4 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-[#A1A1AA]">{t("colQuantity")}</th>
                      <th className="px-4 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-[#A1A1AA]">{t("colUnit")}</th>
                      <th className="px-4 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-[#A1A1AA] hidden sm:table-cell">{t("colSpecification")}</th>
                      <th className="px-4 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-[#A1A1AA]">{t("colConfidence")}</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <>{/* Category header row */}
                        ...
                        {items.map((q, i) => (
                          <tr key={`${category}-${i}`} className={cn("border-b border-[#27272A]", editMode ? "bg-[#F97316]/5" : "hover:bg-[#27272A]/30")}>
                            ...
                            <td className="px-4 py-2 text-xs text-[#A1A1AA]">
                              {editMode ? (
                                <input
                                  type="text"
                            ...
                            <td className="px-4 py-2 text-xs text-[#A1A1AA] hidden sm:table-cell">
                              {editMode ? (
                                <input
                                  type="text"
          ...
          {result.observations.length > 0 && (
            <div className="rounded-lg border border-[#27272A] bg-[#0F0F11] p-4">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-[#A1A1AA] mb-3">{t("observationsLabel")}</h3>
          ...
          {result.summary && (
            <div className="rounded-lg border border-[#27272A] bg-[#F97316]/10 p-4">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-[#A1A1AA] mb-2">{t("summaryLabel")}</h3>
          ...
          {false && (result?.quantities?.length ?? 0) > 0 && (
            <div className="rounded-lg border-2 border-dashed border-brand/30 bg-brand/5 p-6 text-center">
              ...
              <p className="text-xs text-[#A1A1AA] mt-1 mb-4 max-w-md mx-auto">
