  ...
  if (offers.length === 0) {
    return (
      <div className="text-center py-16">
        <Gavel className="h-12 w-12 text-[#71717A] mx-auto mb-3" />
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-[#0F0F11] border border-[#27272A] rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-[#FAFAFA]">{offers.length}</div>
          <div className="text-xs text-[#71717A] mt-1">{t("offersReceived", { count: offers.length, total: offers.length }).split("/")[0]}</div>
        </div>
        <div className="bg-[#0F0F11] border border-[#27272A] rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-emerald-600">
            {offers.filter((o) => o.status === "awarded").length > 0 ? "1" : "0"}
          </div>
          <div className="text-xs text-[#71717A] mt-1">{t("awarded")}</div>
        </div>
        <div className="bg-[#0F0F11] border border-[#27272A] rounded-lg p-4 text-center">
          ...
          <div className="text-xs text-[#71717A] mt-1">{t("savings")}</div>
        </div>
      </div>
      ...
      {offers.map((offer) => {
        ...
        return (
          <div key={offer.id} className={`bg-[#0F0F11] border rounded-lg overflow-hidden ${isAwarded ? "border-emerald-300" : "border-[#27272A]"}`}>
            <div className={`px-4 py-3 flex items-center justify-between ${isAwarded ? "bg-emerald-500/10" : "bg-[#27272A]"} border-b`}>
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-[#FAFAFA]">{formatCHF(offer.total_amount || 0)}</div>
            ...
            <div className="p-4">
              <div className="grid grid-cols-4 gap-4 text-xs text-[#71717A]">
                <div>
                  <div className="font-medium text-[#71717A]">{t("respondedAt")}</div>
                  <div>{new Date(offer.received_at).toLocaleDateString("fr-CH")}</div>
                </div>
                <div>
                  <div className="font-medium text-[#71717A]">{t("deadline")}</div>
                  <div>{offer.validity_date ? new Date(offer.validity_date).toLocaleDateString("fr-CH") : "\u2014"}</div>
                </div>
                <div>
                  <div className="font-medium text-[#71717A]">Paiement</div>
                  <div>{offer.payment_terms || "\u2014"}</div>
                </div>
                <div>
                  <div className="font-medium text-[#71717A]">{t("negotiationRound", { round: offer.negotiation_round })}</div>
                  <div>{offer.is_final ? "Offre finale" : "En cours"}</div>
                </div>
              </div>
              {offer.conditions_text && (
                <div className="mt-3 text-xs text-[#71717A] bg-[#27272A] px-3 py-2 rounded">
                  {offer.conditions_text}
                </div>
              )}
              {!isAwarded && offer.status !== "rejected" && (
                <div className="flex gap-2 mt-4">
                  ...
                  <button className="text-xs px-3 py-1.5 border border-[#27272A] rounded-md hover:bg-[#27272A] text-[#FAFAFA]">
                    {t("startNegotiation")}
                  </button>
                </div>
              )}
