    ...
    <div>
      ...
      {previewOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setPreviewOpen(null)}>
          <div className="w-full max-w-2xl rounded-lg bg-[#18181B] shadow-xl max-h-[80vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            {loadingPreview ? (
              ...
              <>
                <h4 className="text-sm font-semibold text-[#FAFAFA] mb-1">A : {previewData.to}</h4>
                <h4 className="text-sm font-semibold text-[#FAFAFA] mb-3">Sujet : {previewData.subject}</h4>
                <pre className="whitespace-pre-wrap text-xs text-[#FAFAFA] bg-[#27272A] rounded-lg p-4 font-mono">
                  {previewData.body_text}
                </pre>
                <div className="mt-4 flex justify-end">
                  <button onClick={() => setPreviewOpen(null)} className="rounded-md bg-[#F97316] px-4 py-2 text-sm font-medium text-[#F97316]-foreground">
                    Fermer
                  </button>
                </div>
              </>
