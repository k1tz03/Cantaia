        ...
        {priceRequests.map((pr) => {
          ...
          const statusColor =
            pr.status === "responded" ? "border-l-green-500 bg-green-500/10/30"
              ...
              : pr.status === "opened" ? "border-l-blue-500 bg-primary/10/30"
                : "border-l-amber-500 bg-amber-500/10/30";
          return (
            <div key={pr.id} className={`bg-background border border-border border-l-4 rounded-lg p-4 ${statusColor}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div>
                    <div className="text-sm font-medium text-foreground">{supplier?.company_name}</div>
                    <div className="text-xs text-muted-foreground">{supplier?.contact_name} · {supplier?.email}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {isOverdue && (
                  ...
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                    pr.status === "responded" ? "bg-green-500/10 text-green-700 dark:text-green-400"
                      : pr.status === "opened" ? "bg-primary/10 text-primary"
              ...
              <div className="flex items-center gap-6 mt-3 text-xs text-muted-foreground">
                ...
                {daysSinceSent > 0 && pr.status !== "responded" && (
                  <span className="text-muted-foreground">{t("trackingDays", { days: daysSinceSent })}</span>
                )}
              </div>
              ...
              {pr.status !== "responded" && (
                <div className="flex items-center gap-2 mt-3">
                  ...
                  {pr.reminder_enabled !== false && (
                    <button className="text-[10px] text-muted-foreground hover:text-red-500 ml-auto">
                      {t("disableReminder")}
                    </button>
                  )}
                </div>
