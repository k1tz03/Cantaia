      ...
      <div className="flex items-center gap-0.5">
        ...
        <div className="relative">
          <button
            ...
            disabled={!!processing}
            className="rounded p-1 text-amber-600 transition-colors hover:bg-amber-50 disabled:opacity-50"
            ...
            <div className="absolute right-0 top-full z-30 mt-1 w-36 rounded-md border border-slate-200 bg-white py-1 shadow-lg">
              {SNOOZE_OPTIONS.map((opt) => (
                <button
                  ...
                  className="flex w-full px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-50"
                >
                  {t(`snooze_${opt.key}`)}
                </button>
              ))}
      ...
      <div className="grid grid-cols-2 gap-2">
        {actions.map((a) => {
          ...
          return (
            <button
              ...
              disabled={!!processing}
              className={cn(
                "flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-medium transition-colors disabled:opacity-50",
                a.color
              )}
            >
              {isLoading ? (
        ...
        <div className="relative">
          <button
            ...
            disabled={!!processing}
            className="flex w-full items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-amber-600 transition-colors hover:bg-amber-50 disabled:opacity-50"
          >
            {processing === "snooze" ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ) : (
          ...
          {showSnooze && (
            <div className="absolute left-0 top-full z-30 mt-1 w-full rounded-md border border-slate-200 bg-white py-1 shadow-lg">
              {SNOOZE_OPTIONS.map((opt) => (
                <button
                  key={opt.key}
        ...
        <div className="relative">
          <button
            ...
            disabled={!!processing}
            className="flex w-full items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-50 disabled:opacity-50"
          >
          ...
          {showReclassify && (
            <div className="absolute left-0 top-full z-30 mt-1 w-full max-h-48 overflow-y-auto rounded-md border border-slate-200 bg-white py-1 shadow-lg">
              {projects.map((p) => (
                <button
                  key={p.id}
        ...
        <button
          ...
          disabled={!!processing}
          className="col-span-2 flex items-center justify-center gap-1.5 rounded-md border border-dashed border-slate-300 bg-white px-3 py-2 text-xs font-medium text-slate-500 transition-colors hover:bg-slate-50 disabled:opacity-50"
        >
          {processing === "archive" ? (
            <Loader2 className="h-3.5 w-3.5 animate-spin" />
          ) : (
