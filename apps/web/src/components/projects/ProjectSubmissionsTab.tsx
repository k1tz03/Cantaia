...
const statusConfig: Record<string, { label: string; className: string }> = {
  pending: { label: "En attente", className: "bg-[#27272A] text-[#A1A1AA]" },
      ...
      {submissions.length > 0 ? (
          ...
          {submissions.map((sub) => {
            ...
            return (
              ...
              >
                ...
                <ChevronRight className="h-4 w-4 text-[#A1A1AA] group-hover:text-[#A1A1AA] shrink-0" />
              </Link>
            );
          })}
        </div>
        ...
        <div className="flex h-48 items-center justify-center rounded-md border border-dashed border-[#27272A] bg-[#0F0F11]">
          <div className="text-center">
            <FileSpreadsheet className="mx-auto h-10 w-10 text-[#A1A1AA]" />
            <p className="mt-3 text-sm font-medium text-[#A1A1AA]">Aucune soumission</p>
            <p className="text-xs text-[#A1A1AA] mt-1">Importez un descriptif pour commencer</p>
          </div>
        </div>
      )}
    </div>
