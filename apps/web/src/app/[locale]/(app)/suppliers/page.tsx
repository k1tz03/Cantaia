...
const STATUS_CONFIG: Record<SupplierStatus, { color: string; dotColor: string }> = {
  ...
  preferred: { color: "text-primary bg-primary/10", dotColor: "bg-primary/100" },
  blacklisted: { color: "text-red-700 dark:text-red-400 bg-red-500/10", dotColor: "bg-red-500" },
  inactive: { color: "text-muted-foreground bg-muted", dotColor: "bg-muted-foreground" },
...
const SPECIALTY_COLORS: Record<string, string> = {
  ...
  sanitaire: "bg-primary/10 text-primary",
  ...
  serrurerie: "bg-muted text-foreground",
  ...
  demolition: "bg-muted text-foreground",
  ...
  return (
    <>
      <div className="flex h-full">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 border border-border rounded-md hover:bg-muted text-sm font-medium text-foreground"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-md hover:bg-primary/90 text-sm font-medium"
                >
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-border rounded-md text-sm bg-background focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
              </div>
              <select
            ...
            {filtered.length === 0 ? (
              suppliers.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <h3 className="text-lg font-medium text-foreground mb-1">{t("noSuppliers")}</h3>
                  <p className="text-sm text-muted-foreground max-w-sm">{t("noSuppliersDesc")}</p>
                </div>
              )
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted">
                      <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase">{t("companyName")}</th>
                      <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase">{t("specialties")}</th>
                      <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase">{t("geoZone")}</th>
                      <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase">{t("status")}</th>
                    </tr>
                  </thead>
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div className="h-9 w-9 rounded-full bg-primary flex items-center justify-center text-white text-xs font-medium shrink-0">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex flex-wrap gap-1">
                              {supplier.specialties.slice(0, 2).map((sp) => (
                                <span key={sp} className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium ${SPECIALTY_COLORS[sp] || "bg-muted text-muted-foreground"}`}>
                                  {getSpecialtyLabel(sp)}
                                </span>
                              ))}
                              {supplier.specialties.length > 2 && (
                                <span className="text-[11px] text-muted-foreground">+{supplier.specialties.length - 2}</span>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3 text-sm text-muted-foreground">{supplier.geo_zone || "\u2014"}</td>
                          <td className="px-4 py-3">
        ...
        {/* Detail panel */}
        {selected && (
          <div className="w-[400px] shrink-0 sticky top-0 self-start max-h-screen overflow-y-auto bg-background border-l border-border">
            <div className="p-6">
              {/* Header */}
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-full bg-primary flex items-center justify-center text-white text-sm font-medium">
                    {selected.company_name.substring(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-lg font-bold text-foreground">{selected.company_name}</h2>
                      {(() => {
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    disabled={enriching}
                    className="p-1.5 hover:bg-primary/10 rounded text-muted-foreground hover:text-primary disabled:opacity-50"
                  ...
                  <button
                    ...
                    disabled={deleting}
                    className="p-1.5 hover:bg-red-500/10 rounded text-muted-foreground hover:text-red-600 disabled:opacity-50"
                    title="Desactiver"
                  >
                    <Trash2 className="h-4 w-4" />
                  </button>
                  <button onClick={() => setSelectedSupplier(null)} className="p-1.5 hover:bg-muted rounded text-muted-foreground hover:text-muted-foreground">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              </div>
              ...
              {enrichResult && (
                <div className="mb-4 flex items-center justify-between rounded-md bg-primary/10 px-3 py-2 text-sm text-primary ring-1 ring-inset ring-blue-200">
                  <span>{enrichResult}</span>
                  <button
                    onClick={() => setEnrichResult(null)}
                    className="ml-2 rounded p-0.5 hover:bg-primary/10 text-blue-400 hover:text-primary"
                  >
              ...
              <div className="bg-muted rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-muted-foreground disabled:opacity-50"
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground">{t("avgResponseDays")}</span>
              ...
              <div className="space-y-3 mb-6">
                ...
                {selected.website && (
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <a href={selected.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{selected.website}</a>
                  </div>
                )}
              </div>
              ...
              {/* Specialties */}
              <div className="mb-6">
                <h3 className="text-sm font-medium text-foreground mb-2">{t("specialties")}</h3>
                <div className="flex flex-wrap gap-1.5">
                  {selected.specialties.map((sp) => (
                    <span key={sp} className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-medium ${SPECIALTY_COLORS[sp] || "bg-muted text-muted-foreground"}`}>
                      {getSpecialtyLabel(sp)}
                    </span>
                  ))}
                </div>
              ...
              {selected.cfc_codes.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-medium text-foreground mb-2">{t("cfcCodes")}</h3>
                  <div className="flex flex-wrap gap-1.5">
                    {selected.cfc_codes.map((code) => (
                      <span key={code} className="inline-flex items-center px-2.5 py-1 rounded bg-muted text-foreground text-xs font-mono">
                        {code}
                      </span>
                    ))}
                  </div>
              ...
              {selected.certifications.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-medium text-foreground mb-2">{t("certifications")}</h3>
                  <div className="flex flex-wrap gap-1.5">
              ...
              <div className="border-t border-border pt-4">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <div className="text-lg font-semibold text-foreground">{selected.total_requests_sent}</div>
                    <div className="text-xs text-muted-foreground">{t("totalRequests")}</div>
                  </div>
                  <div>
                    <div className="text-lg font-semibold text-foreground">{selected.total_offers_received}</div>
                    <div className="text-xs text-muted-foreground">{t("totalOffers")}</div>
                  </div>
                  <div>
                    <div className="text-lg font-semibold text-foreground">{selected.total_projects_involved}</div>
                    <div className="text-xs text-muted-foreground">{t("totalProjects")}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-border pt-4 mt-4">
                <h3 className="text-sm font-medium text-foreground mb-3">Prix & Offres</h3>
                {pricesLoading ? (
                        ...
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            <span className={`inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-medium ${
                              offer.source_type === "pdf" ? "bg-purple-500/10 text-purple-600" : "bg-primary/10 text-primary"
                            }`}>
              ...
              {selected.notes && (
                <div className="border-t border-border pt-4 mt-4">
                  <h3 className="text-sm font-medium text-foreground mb-2">{t("notes")}</h3>
