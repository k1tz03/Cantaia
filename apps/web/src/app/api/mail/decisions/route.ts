...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: infoEmails } = await (admin as any)
      .from("email_records")
      .select("id, subject, sender_email, sender_name, recipients, body_preview, body_html, body_text, received_at, classification, ai_summary, project_id, is_processed, price_extracted, email_category, suggested_project_data, classification_status")
      .eq("user_id", user.id)
      .eq("is_processed", false)
