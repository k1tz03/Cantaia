...
export async function GET() {
  try {
    ...
    const { data: actionEmails } = await (admin as any)
      .from("email_records")
      .select("id, subject, sender_email, sender_name, body_preview, body_html, body_text, received_at, updated_at, classification, ai_summary, ai_classification_confidence, project_id, is_processed, outlook_message_id, price_extracted, email_category")
      .eq("user_id", user.id)
    ...
    const { data: infoEmails } = await (admin as any)
      .from("email_records")
      .select("id, subject, sender_email, sender_name, body_preview, body_html, body_text, received_at, classification, ai_summary, project_id, is_processed, price_extracted, email_category")
      .eq("user_id", user.id)
      .eq("is_processed", false)
