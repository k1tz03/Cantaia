...
export type ApiActionType =
  ...
  | "calendar_ai_command"
  | "calendar_sync"
  // Ajoutés lors de la campagne de correction 08/2026 (auparavant passés via
  // des casts `as any` à trackApiUsage) : métrage des routes IA nouvelles/renommées.
...
export interface BriefingContent {
   ...
   * - "data": assembled directly from database rows (no model). Honest label
   *   for the fallback path — it must never be reported as "ai".
   * - "fallback": legacy value, kept so briefings stored before the rename
   *   still deserialise.
   */
